...
def test_get_playlist_songs_returns_all_songs_regression(app, playlist_with_songs):
    """Regression test for Issue #5: Ensure all songs (including the last one) are returned."""
    with app.app_context():
        songs = get_playlist_songs(playlist_with_songs)
        assert len(songs) == 3
        assert songs[0]["title"] == "Song 1"
        assert songs[1]["title"] == "Song 2"
        assert songs[2]["title"] == "Song 3"
